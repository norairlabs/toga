...
namespace TOGA
{
    public partial class AddNewPeripheral : Form
    {
        ...
        public bool SerialIsValid = false;
        ...
        private void RollId_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (ValidIds[v] != 1)
            {
                ...
                //IDInUse.Visible = true;   // ID is out off reach of the user for a more simple setup
            }
            else
            {
                ID.BackColor = Color.LightGreen;
        ...
        private void DESCRIPTION_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!(InvalidName.Visible) && DESCRIPTION.Text.Length != 0 && SerialIsValid)
            {
                AddPeripheral.Enabled = true;
            }
            else
        ...
        private void SERIALtext_Changed(object sender, EventArgs e)
        {
            string s = SERIAL.Text;
            Debug.WriteLine(s);
            if (SERIAL.Text.Length > 1)
            {
                ...
                try
                {
                    int hardware = int.Parse(SERIAL.Text.Substring(0, 2));
                    switch (hardware)
                    {
                        case 31:
                        ...
                        case 50:
                            ...
                            break;
                        default:
                            HardwareTypeLabel.Text = "Invalid serial number ";
                            SerialIsValid = false;
                            break;
                    }
                }
            ...
            if ((SERIAL.Text.Length) == 10 && (SerialIsValid == true))
            {
                if (DESCRIPTION.Text.Length != 0)
                {
                    AddPeripheral.Enabled = true;
